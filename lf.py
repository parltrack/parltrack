...
def parse(line):
    ...
    try:
        ...
        return line
    if tokens[1] not in ['db','dossier','dossiers','mep','meps','mgr']:
        return line
    module = tokens[1]
...
def dump(date, module, level, levelz, msg):
    ...
    cmod = {
        ...
        "dossier": "dossier",
        "dossiers": "\033[38;2;127;127;127mdossiers\033[0m",
    }
    if dlevel<level:
        return
...
if not fname:
    print('input logfile must be specified')
    usage()
    ...
    for line in f:
        ...
        if tmp==line:
            ...
            if msg.startswith("preserving deleted path ['other'] for obj id:"): # todo remove
                continue
            if module in ['dossier','mep'] and level==3 and msg.startswith("adding "):
                if module not in changes: changes[module]={'added':0, 'updated':0}
                changes[module]['added']+=1
            if module in ['dossier','mep'] and level==3 and msg.startswith("updating "):
                if module not in changes: changes[module]={'added':0, 'updated':0}
